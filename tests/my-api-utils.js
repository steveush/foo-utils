/*!
* MyApi.utils - Contains common utility methods and classes used in our plugins.
* @version 0.0.1
...
*/
/**
 * @file This creates the global MyApi.utils namespace ensuring it only registers itself if the namespace doesn't already exist or if the current version is lower than this one.
 */
(function ($) {

	if (!$){
		console.warn('jQuery must be included in the page prior to the MyApi.utils library.');
		return;
	}
	...
	var utils = {
		 ...
		 * 	jQuery(function($){
	 	 ...
	 	 * 	});
		 * </script>
		 * @example {@caption The reason the above throws an error is that the `$.fn.myPlugin` function is registered to the first instance of jQuery in the page however the instance used to create the ready callback and actually try to execute `$(...).myPlugin()` is the second. To resolve this issue ideally you would remove the second instance of jQuery however you can use the `MyApi.utils.$` member to ensure you are always working with the instance of jQuery the library was registered with.}{@lang html}
		 * <script src="jquery-1.12.4.js"></script>
		 * <script src="my-plugin.js"></script>
		 * <script src="jquery-2.2.4.js"></script>
		 * <script>
		 * 	MyApi.utils.$(function($){
	 ...
	 * console.log( MyApi.utils.versionCompare( "0", "0" ) ); // => 0
	 * console.log( MyApi.utils.versionCompare( "0.0", "0" ) ); // => 0
	 * console.log( MyApi.utils.versionCompare( "0.0", "0.0.0" ) ); // => 0
	 * console.log( MyApi.utils.versionCompare( "0.1", "0.0.0" ) ); // => 1
	 * console.log( MyApi.utils.versionCompare( "0.1", "0.0.1" ) ); // => 1
	 * console.log( MyApi.utils.versionCompare( "1", "0.1" ) ); // => 1
	 * console.log( MyApi.utils.versionCompare( "1.10", "1.9" ) ); // => 1
	 * console.log( MyApi.utils.versionCompare( "1.9", "1.10" ) ); // => -1
	 * console.log( MyApi.utils.versionCompare( "1", "1.1" ) ); // => -1
	 * console.log( MyApi.utils.versionCompare( "1.0.9", "1.1" ) ); // => -1
	 ...
	 * console.log( MyApi.utils.versionCompare( "not-a-version", "1.1" ) ); // => NaN
	 * console.log( MyApi.utils.versionCompare( "1.1", "not-a-version" ) ); // => NaN
	 * console.log( MyApi.utils.versionCompare( "not-a-version", "not-a-version" ) ); // => NaN
	 */
	...
	var exists = !!window.MyApi.utils; // does the namespace already exist?
	if (!exists){
		// if it doesn't exist register it
		window.MyApi.utils = utils;
	} else if (exists){
		...
		if (utils.versionCompare(utils.version, window.MyApi.utils.version) > 0){
			// if it exists but it's an old version replace it
			console.warn("An older version of MyApi.utils (" + window.MyApi.utils.version + ") already exists in the page, version " + utils.version + " will override it.");
			window.MyApi.utils = utils;
		} else {
			// otherwise its a newer version so do nothing
			console.warn("A newer version of MyApi.utils (" + window.MyApi.utils.version + ") already exists in the page, version " + utils.version + " will not register itself.");
		}
	}

	// at this point there will always be a MyApi.utils namespace registered to the global scope.
...
(function ($, _){
	 ...
	 * @memberof MyApi.utils.is
	 * @function array
	 * @param {*} value - The value to check.
	 * @returns {boolean} `true` if the supplied `value` is an array.
	 * @example {@run true}
	 * // alias the MyApi.utils.is namespace
	 * var _is = MyApi.utils.is;
	 *
...
(function($, _, _is){
	...
	/**
	 * @summary The regular expression to test if a function uses the `this._super` method applied by the {@link MyApi.utils.fn.add} method.
	 * @memberof MyApi.utils.fn
	 * @name CONTAINS_SUPER
	 * @type {RegExp}
	 * @default /\b_super\b/
	 * @readonly
	 ...
	 * @example {@run true}
	 * // alias the MyApi.utils.fn namespace
	 * var _fn = MyApi.utils.fn;
	 *
	 ...
	 *
	 * ```javascript
	 * MyApi.utils.fn.addOrOverride( MyClass.prototype, "someMethod", function(arg1, arg2){
	 * 	// execute the original
	 * 	this._super(arg1, arg2);
	 * });
	 * ```
	 *
	 * This method is used by the {@link MyApi.utils.Class} to implement the inheritance of individual methods.
	 * @example {@run true}
	 * // alias the MyApi.utils.fn namespace
	 * var _fn = MyApi.utils.fn;
	 *
	 ...
	 * @param {Object} [ctx=window] - If the `value` is a string this is supplied to the {@link MyApi.utils.fn.fetch} method as the content to retrieve the function from.
	 ...
	 *
	 * When just the function name is supplied this method uses the {@link MyApi.utils.fn.fetch} method to resolve and wrap it to ensure when it's called the correct context is applied.
	 *
...
(function(_, _is){
	 ...
	 * @memberof MyApi.utils.url
	 * @function parts
	 * @param {string} url - The url to parse.
	 * @returns {MyApi.utils.url~Parts}
	 * @example {@run true}
	 * // alias the MyApi.utils.url namespace
	 * var _url = MyApi.utils.url;
	 *
	...
	/**
	 * @summary A plain JavaScript object returned by the {@link MyApi.utils.url.parts} method.
	 * @typedef {Object} MyApi.utils.url~Parts
	 ...
	 * @see {@link MyApi.utils.url.parts} for example usage.
	 */
...
(function (_, _is, _fn) {
	 ...
	 * @memberof MyApi.utils.str
	 * @function camel
	 * @param {string} target - The string to camel case.
	 * @returns {string}
	 * @example {@run true}
	 * // alias the MyApi.utils.str namespace
	 * var _str = MyApi.utils.str;
	 *
	 ...
	 * @description This method differs from {@link MyApi.utils.str.contains} in that it searches for whole words by splitting the `target` string on word boundaries (`\b`) and then comparing the individual parts.
	 * @example {@run true}
	 * // alias the MyApi.utils.str namespace
	 * var _str = MyApi.utils.str,
	 * 	// create a string to test
...
(function($, _, _is, _fn, _str){
	 ...
	 *
	 * This method makes use of the {@link MyApi.utils.obj.merge} method internally.
	 * @example {@run true}
	 * // alias the MyApi.utils.obj namespace
	 * var _obj = MyApi.utils.obj,
	 ...
	 * @description This does not merge arrays by index as jQuery does, it treats them as a single property and replaces the array with a shallow copy of the new one.
	 *
	 * This method is used internally by the {@link MyApi.utils.obj.extend} method.
	 * @example {@run true}
	 * // alias the MyApi.utils.obj namespace
	 * var _obj = MyApi.utils.obj,
	 * 	// create some objects to merge
	 ...
	 * @function mergeValid
	 * @param {Object} target - The object to merge properties into.
	 * @param {MyApi.utils.obj~Validators} validators - An object containing validators for the `target` object properties.
	 * @param {Object} object - The object containing properties to merge.
	 * @param {MyApi.utils.obj~Mappings} [mappings] - An object containing property name mappings.
	 * @returns {Object} The modified `target` object containing any valid properties from the supplied `object`.
	 * @example {@caption Shows the basic usage for this method and shows how invalid properties or those with no corresponding validator are ignored.}{@run true}
	 * // alias the MyApi.utils.obj and MyApi.utils.is namespaces
	 * var _obj = MyApi.utils.obj,
	 * 	_is = MyApi.utils.is;
	 *
	...
	/**
	 * @summary An object used by the {@link MyApi.utils.obj.mergeValid|mergeValid} method to map new values onto the `target` object.
	 * @typedef {Object.<string,string>|Object.<string,Array.<string>>} MyApi.utils.obj~Mappings
	...
	/**
	 * @summary An object used by the {@link MyApi.utils.obj.mergeValid|mergeValid} method to validate properties.
	 * @typedef {Object.<string,function(*):boolean>} MyApi.utils.obj~Validators
	 * @description The validators object is a single level object. If you want to validate a property of a child object you must supply the name using `.` notation as seen in the below example with the `"name.first"` and `"name.last"` properties.
	 *
	 * Any function that accepts a value to test as the first argument and returns a boolean can be used as a validator. This means the majority of the {@link MyApi.utils.is} methods can be used directly. If the property supports multiple types just provide your own function as seen with `"name.last"` in the below example.
...
(function($, _, _is){
	 ...
	 * @example {@run true}
	 * // alias the MyApi.utils namespace
	 * var _ = MyApi.utils;
	 *
	...
	/**
	 * @summary Remove the id from the given `$element` if it was set using the {@link MyApi.utils.uniqueId|uniqueId} method.
	 * @memberof MyApi.utils
...
(function($, _, _is){
	 ...
	 * @memberof MyApi.utils.transition
	 * @name supported
	 * @type {boolean}
	 */
	 ...
	 * @param {number} [timeout] - The maximum time, in milliseconds, to wait for the `transitionend` event to be raised. If not provided this will be automatically set to the elements `transition-duration` property plus an extra 50 milliseconds.
	 * @returns {jQuery}
	 * @description This exposes the {@link MyApi.utils.transition.start} method as a jQuery plugin.
	 * @example
	 * jQuery( ".selector" ).fooTransition( "expand", true, function(){
	 * 	// do something once the transition is complete
	 * } );
	 * @this jQuery
	 * @see {@link MyApi.utils.transition.start} for more details on the underlying function.
	 * @see {@link https://developer.mozilla.org/en/docs/Web/CSS/transition-duration|transition-duration - CSS | MDN} for more information on the `transition-duration` CSS property.
	 */
...
(function ($, _, _is, _obj, _fn) {
	 ...
	 *
	 * Have a look at the {@link MyApi.utils.Class.extend|extend} and {@link MyApi.utils.Class.override|override} method examples to see some basic usage.
	 * @example {@caption When using this base class the actual construction of a class is performed by the `construct` method.}
	 * var MyClass = MyApi.utils.Class.extend({
	 ...
	 * @memberof MyApi.utils.Class#
	 * @name _super
	 * @type {?function}
	 * @description This is only available within the scope of an overriding method if it was created using the {@link MyApi.utils.Class.extend|extend}, {@link MyApi.utils.Class.override|override} or {@link MyApi.utils.fn.addOrOverride} methods.
	 * @see {@link MyApi.utils.fn.addOrOverride} to see an example of how this property is used.
	 */
	 ...
	 * @description Every class created using this method has both the {@link MyApi.utils.Class.extend|extend} and {@link MyApi.utils.Class.override|override} static methods added to it to allow it to be extended.
	 * @example {@caption The below shows an example of how to implement inheritance using this method.}{@run true}
	 * // create a base Person class
	 * var Person = MyApi.utils.Class.extend({
	 ...
	 * console.log(
	 * 	p instanceof Person && p.constructor === Person && p instanceof MyApi.utils.Class
	 * 	&& n instanceof Ninja && n.constructor === Ninja && n instanceof Person && n instanceof MyApi.utils.Class
	 * ); // => true
	 */
	_.Class.extend = function(definition){
		...
		function Class() {
			if (!_is.fn(this.construct))
				throw new SyntaxError('MyApi.utils.Class objects must be constructed with the "new" keyword.');
	 ...
	 * @description This is a helper method for overriding a single function of a {@link MyApi.utils.Class} or one of its child classes. This uses the {@link MyApi.utils.fn.addOrOverride} method internally and simply provides the correct prototype.
...
(function($, _, _is, _fn){
	...
	_.Factory = _.Class.extend(/** @lends MyApi.utils.Factory */{
		/**
		 * @summary A factory for classes allowing them to be registered and created using a friendly name.
		 * @memberof MyApi.utils
		 * @constructs Factory
		 * @description This class allows other classes to register themselves for use at a later time. Depending on how you intend to use the registered classes you can also specify a load and execution order through the `priority` parameter of the {@link MyApi.utils.Factory#register|register} method.
		 * @augments MyApi.utils.Class
		 * @borrows MyApi.utils.Class.extend as extend
		 * @borrows MyApi.utils.Class.override as override
		 */
		construct: function(){
			/**
			 * @summary An object containing all registered classes.
			 * @memberof MyApi.utils.Factory#
			 * @name registered
		 ...
		 * var factory = new MyApi.utils.Factory();
		 *
		 ...
		 * var Extension = MyApi.utils.Class.extend({
		 ...
		 * var Logger = MyApi.utils.Class.extend({
		 * 	write: function( message ){
		 ...
		 * @param {number} [priority=0] - This determines the index for the class when using either the {@link MyApi.utils.Factory#load|load} or {@link MyApi.utils.Factory#names|names} methods, a higher value equals a lower index.
		 * @returns {boolean} `true` if the `klass` was successfully registered.
		 * @description Once a class is registered you can use either the {@link MyApi.utils.Factory#load|load} or {@link MyApi.utils.Factory#make|make} methods to create new instances depending on your use case.
		 * @example {@run true}
		 * // create a new instance of the factory, this is usually exposed by the class that will be using the factory.
		 * var factory = new MyApi.utils.Factory();
		 *
...
(function(_, _fn, _str){
	...
	_.Debugger = _.Class.extend(/** @lends MyApi.utils.Debugger */{
		/**
		 ...
		 * @function enable
		 * @example
		 * var d = new MyApi.utils.Debugger( "FOO_DEBUG" );
		 * d.log( "Never logged" );
		 * d.enabled();
		 * d.log( "I am logged!" );
		 */
		 ...
		 * @param {*} [argN] - Any number of additional arguments to supply after the message.
		 * @see {@link MyApi.utils.str.format} for more information on supplying the replacements object.
		 */
...
(function($, _, _is){
	...
	_.Throttle = _.Class.extend(/** @lends MyApi.utils.Throttle */{
		/**
		 * @summary A timer to throttle the execution of code.
		 * @memberof MyApi.utils
		 * @constructs
		 * @param {number} [idle=0] - The idle time, in milliseconds, that must pass before executing the callback supplied to the {@link MyApi.utils.Throttle#limit|limit} method.
		 * @augments MyApi.utils.Class
		 * @borrows MyApi.utils.Class.extend as extend
		 * @borrows MyApi.utils.Class.override as override
		 ...
		 *
		 * Take a look at the examples for the {@link MyApi.utils.Throttle#limit|limit} and {@link MyApi.utils.Throttle#clear|clear} methods for basic usage.
		 ...
		 * var throttle = new MyApi.utils.Throttle( 50 );
		 *
		...
		construct: function(idle){
			...
			this.active = false;
			/**
			 * @summary The idle time, in milliseconds, the timer should wait before executing the callback supplied to the {@link MyApi.utils.Throttle#limit|limit} method.
			 * @type {number}
			 * @readonly
			 * @default 0
			 */
		 ...
		 * @example <caption>In the below example the <code>callback</code> function will only be executed once despite the repeated calls to the {@link MyApi.utils.Throttle#limit|limit} method as each call resets the idle timer.</caption>{@run true}
